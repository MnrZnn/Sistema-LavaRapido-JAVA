...
public class GestorController {
    ...
    public String editarServicoForm(@PathVariable String id, Model model) {
        model.addAttribute("servico", servicoService.buscarPorId(id).orElseThrow());
        model.addAttribute("servicos", servicoService.listarTodos());
        return "gestor/servicos";
    }
    ...
    public String editarAgendamento(@PathVariable String id, Model model) {
        model.addAttribute("agendamento", agendamentoService.buscarPorId(id).orElseThrow());
    ...
    public String estatisticas(@RequestParam(required = false) Integer ano,
        ...
        for (int i = 5; i >= 0; i--) {
            ...
            BigDecimal valor = agendamentoService.receitaPeriodo(ym.atDay(1), ym.atEndOfMonth());
            receitaMensal.put(chave, valor);
        }

        Map<LocalDate, Long> lavagensNaSemana = agendamentoService.contagemPorDia(inicioSemana, fimSemana);
        ...
        model.addAttribute("lavagensNaSemana", lavagensNaSemana);
